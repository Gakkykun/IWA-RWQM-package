Program main
  ...
  integer :: npar, max_iter=25, min_iter = 10
  ...
  allocate(par(npar), minpar(npar), maxpar(npar), w_y(npar+1), w_p(npar+1, npar),  &
    ...
    K1 = 2000d0; Khyd = 3.0d0; 
    KS_Haer=2.0d0; KS_Anox = 2.0d0 ; Kgrowth_ALG = 2.0d0 
    
    par(:) = [K1, Khyd, KS_Haer, KS_Anox, Kgrowth_ALG]
    minpar(:) = [1000d0, 1.0d0, 1.0d0, 1.0d0, 1.0d0] 
...
! Calculate PBAR, the centroid of the simplex vertices 
    ...
    call fun(w_pr(:), minpar(:), maxpar(:), npar, ypr, w_y(hi), dataset_number)
    ...
    if ( ypr < w_y(low)) then    
      ...
      call fun(w_prr(:), minpar(:), maxpar(:), npar, yprr, w_y(hi), dataset_number)
          ...
          if ( yprr < w_y(hi)) then
            ...
            do j = 1, npar+1
              if (j /= low) then
                ...
                call fun(w_p(j, :), minpar(:), maxpar(:), npar, w_y(j), w_y(hi), dataset_number)
...
contains
subroutine fun(par, min, max, npar, fval, fref, dataset_number)
  ...
  ! fval = output(npar, ptrans(:), dataset_number)
